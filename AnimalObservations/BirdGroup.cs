...
namespace AnimalObservations
{
    public class BirdGroup
    {
        ...
        static Dictionary<Guid, BirdGroup> BirdGroups = new Dictionary<Guid, BirdGroup>();
        ...
        public static BirdGroup FromGuid(Guid guid)
        {
            ...
            birdGroup.Feature = MobileUtilities.GetFeature(FeatureLayer, guid);
            if (birdGroup.Feature == null)
                return null;
        ...
        public static BirdGroup CreateWith(Observation observation)
        {
            ...
            BirdGroup birdGroup = new BirdGroup();
            birdGroup.Feature = MobileUtilities.CreateNewFeature(FeatureLayer);
            birdGroup.Guid = new Guid(birdGroup.Feature.FeatureDataRow.GlobalId.ToByteArray()); 
        ...
        public Point Location
        {
            get
            {
                return LocationByBoat;
                //return LocationByTransect;
            }
        }

        private Point LocationByBoat
        {
            get
            {
                double gps_X = Observation.GpsPoint.Location.X;
                double gps_Y = Observation.GpsPoint.Location.Y;
                ...
                double X = gps_X + Observation.Distance * Math.Cos(bearing);
                double Y = gps_Y + Observation.Distance * Math.Sin(bearing);
                return new Point(X, Y);
            }
        }

        private Point LocationByTransect
        {
            get
            {
                double gps_X = Observation.GpsPoint.Location.X;
